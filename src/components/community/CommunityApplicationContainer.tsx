...
import CommunityApplicationProgress from "./CommunityApplicationProgress";
import CommunityApplicationQuestion from "./CommunityApplicationQuestion";
import { saveCommunityApplication } from "@/lib/community/saveCommunityApplication";

/**
 * Main container for the community application
 */
const CommunityApplicationContainer: React.FC = () => {
  const { state, dispatch } = useCommunityApplication();
  const { currentQuestion, isComplete, answers, isSubmitting, submitError } =
    state;
  const router = useRouter();
  const [hasSubmitted, setHasSubmitted] = useState(false);

  // Handle application completion and submission
  useEffect(() => {
    const handleSubmission = async () => {
      if (isComplete && !hasSubmitted && !isSubmitting) {
        ...
        try {
          const { success, error } = await saveCommunityApplication(answers);

          if (success) {
            dispatch({ type: "SUBMIT_SUCCESS" });
            // Redirect to confirmation page after a brief delay
            setTimeout(() => {
              router.push("/community/apply/confirmation");
            }, 1000);
          } else {
            dispatch({
              type: "SUBMIT_ERROR",
              payload: "Failed to submit your application. Please try again.",
            });
            setHasSubmitted(false);
          }
        } catch (err) {
          console.error("Error submitting community application:", err);
          dispatch({
            type: "SUBMIT_ERROR",
    ...
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-600">
            Your application has been submitted successfully. Redirecting you
            now...
          </p>
        </div>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <CommunityApplicationProgress />
      <div className="mt-8">
        <CommunityApplicationQuestion />
      </div>
    </div>
  );
};

export default CommunityApplicationContainer;